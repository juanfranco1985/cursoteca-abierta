  ...
  app.innerHTML = `
    ...
    <section class="section grid two">
      <div class="card danger-card">
        ...
        <p>Rutina breve para aplicar lo aprendido con un caso pequeno, revisar resultados y anotar pendientes.</p>
        <a class="btn danger" href="#/emergency">Abrir Practica rapida</a>
      </div>
      <div class="card">
        <p class="eyebrow">Rumbo v0.6</p>
        <h2>Recursos del curso</h2>
        <p>El curso incluye modulos, quizzes, checklists, proyectos guiados, progreso local y constancia educativa.</p>
        <a class="btn ghost" href="#/template">Ver recursos</a>
      </div>
    </section>
...
function renderModuleCard(module) {
  ...
  return `
    <article class="card module-card">
      <div class="meta"><span class="pill">${esc(module.id.toUpperCase())}</span><span class="pill warn">${esc(module.learningRisk || "Punto clave")}</span></div>
...
function renderModules() {
  app.innerHTML = `
    ...
    <input class="search-box" id="moduleSearch" placeholder="Buscar modulo por tema, concepto o practica..." aria-label="Buscar módulo" />
...
function renderChecklists() {
  const state = storage.read();
  app.innerHTML = `
    <section class="section-head"><div><p class="eyebrow">Autoauditoría</p><h1>${esc(manifest.labels?.checklists || "Checklists prácticos")}</h1><p>Marca acciones reales de aprendizaje. El avance queda guardado en este navegador.</p></div></section>
...
function renderIncidents() {
  app.innerHTML = `
    <section class="section-head"><div><p class="eyebrow">Simulador de casos</p><h1>${esc(manifest.labels?.incidents || "Incidentes")}</h1><p>Proyectos y practicas guiadas para aplicar el curso con casos pequenos, verificables y seguros.</p></div></section>
    <input class="search-box" id="incidentSearch" placeholder="Buscar caso, practica o proyecto..." />
...
function renderIncident(id) {
  ...
  app.innerHTML = `
    <article class="article">
      ...
      <div class="actions"><a class="btn ghost" href="#/incidents">Volver a casos</a><a class="btn danger" href="#/emergency">Practica rapida</a></div>
    </article>`;
}
...
function renderEmergency() {
  app.innerHTML = `
    <section class="article">
      <div class="card danger-card"><p class="eyebrow">Practica rapida</p><h1>Practica rapida del curso</h1><p class="lead">Esta vista ordena una practica breve para repasar el curso. No reemplaza fuentes oficiales, revision docente ni asesoramiento profesional cuando corresponda.</p></div>
      <div class="card"><h2>Primeros 5 pasos</h2>${list(["Elegir un objetivo pequeno y concreto.", "Trabajar con ejemplos educativos y sin datos sensibles.", "Anotar supuestos, dudas y fuentes consultadas.", "Probar el resultado antes de compartirlo.", "Guardar una breve evidencia de aprendizaje."])}</div>
      <div class="card"><h2>Durante la practica</h2>${list(["Revisar el ejemplo paso a paso y comparar con la leccion.", "Corregir errores detectados antes de avanzar.", "Probar una variante pequena del mismo ejercicio.", "Consultar una fuente confiable si aparece una duda.", "Escribir una conclusion breve con lo aprendido."])}</div>
      <div class="card"><h2>Despues de practicar</h2>${list(["Ordenar notas, ejemplos y resultados.", "Revisar que el resultado responda al objetivo inicial.", "Actualizar el glosario personal con conceptos nuevos.", "Preparar una mejora para la siguiente practica.", "Consultar fuentes oficiales o material especializado si corresponde."])}</div>
...
function renderLegal() {
  app.innerHTML = `
    <section class="article">
      ...
      <div class="card"><h2>Límites responsables</h2><p>Las recomendaciones ayudan a ordenar el aprendizaje, pero no reemplazan fuentes oficiales, revision docente ni asesoramiento profesional.</p></div>
    </section>`;
}
...
function renderCertificate() {
  ...
  let level = "Aprendiz en formacion";
  if (p >= 90 && quizAvg >= 80) level = "Aprendizaje avanzado";
  else if (p >= 70 && quizAvg >= 70) level = "Aprendizaje solido";
  else if (p >= 45) level = "Aprendizaje en progreso";
...
function renderTemplateGuide() {
  app.innerHTML = `
    <section class="article">
      <div class="card"><p class="eyebrow">Arquitectura clonable</p><h1>Cómo crear otro curso rápido</h1><p>Esta v0.6 convierte el proyecto en una Recursos del curso oficial: el motor web queda separado del contenido, la identidad del curso vive en el manifest y los datos pedagógicos se reemplazan por JSON.</p></div>
