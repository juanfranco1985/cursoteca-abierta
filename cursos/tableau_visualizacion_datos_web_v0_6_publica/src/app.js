...
let checklists = [];
let incidents = [];
let visualizationLab = { datasets: [], visualQuestions: [], chartTypes: [] };
let dashboardStoryboard = { objectives: [], dashboardTemplates: [] };
let storytellingLab = { scenarios: [], misleadingExamples: [], clarityChecklist: [] };
...
async function bootstrap() {
  try {
    ...
    const [courseData, checklistData, incidentData, visualizationData, storyboardData, storytellingData] = await Promise.all([
      loadJson(manifest.dataPaths.course),
      loadJson(manifest.dataPaths.checklists),
      loadJson(manifest.dataPaths.incidents),
      manifest.dataPaths.visualizationLab ? loadJson(manifest.dataPaths.visualizationLab) : Promise.resolve({ datasets: [], visualQuestions: [], chartTypes: [] }),
      manifest.dataPaths.dashboardStoryboard ? loadJson(manifest.dataPaths.dashboardStoryboard) : Promise.resolve({ objectives: [], dashboardTemplates: [] }),
      manifest.dataPaths.storytellingLab ? loadJson(manifest.dataPaths.storytellingLab) : Promise.resolve({ scenarios: [], misleadingExamples: [], clarityChecklist: [] }),
    ]);
    course = courseData;
    checklists = checklistData.checklists || [];
    incidents = incidentData.incidents || [];
    visualizationLab = visualizationData || { datasets: [], visualQuestions: [], chartTypes: [] };
    dashboardStoryboard = storyboardData || { objectives: [], dashboardTemplates: [] };
    storytellingLab = storytellingData || { scenarios: [], misleadingExamples: [], clarityChecklist: [] };
...
function render() {
  ...
  const routes = {
    ...
    "visualization-lab": renderVisualizationLab,
    "visual-question": () => renderVisualQuestion(p1),
    "dashboard-storyboard": renderDashboardStoryboard,
    "storyboard-objective": () => renderStoryboardObjective(p1),
    "storyboard-report": renderStoryboardReport,
    "storytelling": renderStorytelling,
    "visual-presentation": () => renderVisualPresentation(p1),
    "misleading-visuals": renderMisleadingVisuals,
    "visual-clarity-checklist": renderVisualClarityChecklist,
  };
  ...
  app.innerHTML = `
    <section class="hero">
      <div class="hero-card">
        ...
        <div class="actions">
          ...
          <a class="btn ghost" href="#/visualization-lab">Laboratorio visual</a>
          <a class="btn ghost" href="#/dashboard-storyboard">Constructor storyboard</a>
          <a class="btn ghost" href="#/storytelling">Storytelling visual</a>
        </div>
      </div>
      <aside class="card">
    ...
    <section class="section card compact-helper">
      <div>
        ...
        <h2>Storytelling visual y claridad</h2>
        <p>Se agregó simulador de presentación visual, ejemplos de visualizaciones engañosas, checklist de claridad y mejoras móviles para el laboratorio.</p>
      </div>
      <a class="btn ghost" href="#/storytelling">Abrir storytelling</a>
    </section>`;
}
...
function getVisualDataset(id) {
  return (visualizationLab.datasets || []).find(d => d.id === id);
}

function renderMiniTable(dataset) {
...
function renderVisualizationLab() {
  const questions = visualizationLab.visualQuestions || [];
  app.innerHTML = `
    <section class="article wide-article">
      <div class="card">
        <p class="eyebrow">Laboratorio v0.2</p>
        <h1>Laboratorio de visualizaciones</h1>
        <p class="lead">Elegí una pregunta visual, revisá el dataset y compará una visualización recomendable contra una opción engañosa o poco útil.</p>
      </div>
      <div class="grid two section">
        ${(visualizationLab.datasets || []).map(dataset => `
          <article class="card">
            <p class="eyebrow">Dataset</p>
            <h2>${esc(dataset.title)}</h2>
            <p>${esc(dataset.description)}</p>
            <div class="pill-row">${(dataset.columns || []).map(c => `<span class="pill">${esc(c)}</span>`).join("")}</div>
          </article>`).join("")}
      </div>
      <div class="section-head"><div><p class="eyebrow">Preguntas visuales</p><h2>Elegí qué querés responder</h2></div></div>
      <div class="list">${questions.map(q => `
        <a class="list-row" href="#/visual-question/${q.id}">
          <span><strong>${esc(q.question)}</strong><br><small>${esc(getVisualDataset(q.datasetId)?.title || "Dataset")}</small></span>
          <span class="pill ok">Ver recomendación</span>
        </a>`).join("")}</div>
      <div class="card section">
        <h2>Tipos de gráfico incluidos</h2>
        <div class="grid two">${(visualizationLab.chartTypes || []).map(type => `<article class="stat"><strong>${esc(type.name)}</strong><span>${esc(type.bestFor)}</span><p><small>Evitar cuando: ${esc(type.avoidWhen)}</small></p></article>`).join("")}</div>
      </div>
    </section>`;
}

function renderVisualQuestion(questionId) {
  const q = (visualizationLab.visualQuestions || []).find(item => item.id === questionId);
  if (!q) return renderNotFound();
  const dataset = getVisualDataset(q.datasetId);
  const report = [
    `Pregunta visual: ${q.question}`,
    `Dataset: ${dataset?.title || q.datasetId}`,
    `Visualización recomendada: ${q.recommendedChart}`,
    `Visualización a evitar: ${q.badChart}`,
    `Justificación: ${q.reason}`,
    `Próximo paso: construir primero una vista simple, validar si responde la pregunta y luego agregar filtros o color solo si aportan claridad.`
  ].join("\n");
  app.innerHTML = `
    <section class="article wide-article">
      <div class="card">
        <p class="eyebrow">Pregunta visual</p>
        <h1>${esc(q.question)}</h1>
        <p class="lead">Antes de elegir un gráfico, definí qué pregunta debe responder la visualización.</p>
        <div class="actions"><a class="btn ghost" href="#/visualization-lab">Volver al laboratorio</a><button class="btn primary" id="copyVisualReport">Copiar reporte</button><button class="btn ghost" id="downloadVisualReport">Descargar .txt</button></div>
      </div>
      <div class="grid two section">
        <article class="card ok-card">
          <p class="eyebrow">Recomendado</p>
          <h2>${esc(q.recommendedChart)}</h2>
          <p>${esc(q.reason)}</p>
        </article>
        <article class="card danger-card">
          <p class="eyebrow">Evitar o revisar</p>
          <h2>${esc(q.badChart)}</h2>
          <p>Puede dificultar la lectura, ocultar tendencia, exagerar proporciones o no responder la pregunta principal.</p>
        </article>
      </div>
      <div class="card section">
        <h2>Vista previa del dataset</h2>
        <p>${esc(dataset?.description || "Dataset de ejemplo")}</p>
        ${renderMiniTable(dataset)}
      </div>
      <div class="card section">
        <h2>Mini reporte visual</h2>
        <pre><code>${esc(report)}</code></pre>
      </div>
    </section>`;
  document.querySelector("#copyVisualReport")?.addEventListener("click", async () => { await navigator.clipboard.writeText(report); alert("Reporte copiado."); });
  document.querySelector("#downloadVisualReport")?.addEventListener("click", () => downloadText(`reporte_visual_${q.id}.txt`, report));
}


function storyboardStorageKey() {
  const id = manifest?.courseId || "curso-web";
  return `curso-web:${id}:storyboard-history:v1`;
}

function readStoryboardHistory() {
  try { return JSON.parse(localStorage.getItem(storyboardStorageKey())) || []; } catch { return []; }
}

function writeStoryboardHistory(items) {
  localStorage.setItem(storyboardStorageKey(), JSON.stringify(items.slice(0, 12)));
}

function getStoryboardObjective(id) {
  return (dashboardStoryboard.objectives || []).find(item => item.id === id);
}

function getDashboardTemplate(id) {
  return (dashboardStoryboard.dashboardTemplates || []).find(item => item.id === id);
}

function renderDashboardStoryboard() {
  const objectives = dashboardStoryboard.objectives || [];
  const templates = dashboardStoryboard.dashboardTemplates || [];
  const history = readStoryboardHistory();
  app.innerHTML = `
    <section class="article wide-article">
      <div class="card">
        <p class="eyebrow">Laboratorio v0.3</p>
        <h1>Constructor de dashboards y storyboards</h1>
        <p class="lead">Elegí un objetivo visual, revisá el dataset recomendado y armá un orden narrativo para explicar el hallazgo con claridad.</p>
        <div class="actions">
          <a class="btn ghost" href="#/visualization-lab">Laboratorio visual</a>
          <a class="btn ghost" href="#/storyboard-report">Reporte e historial</a>
        </div>
      </div>
      <div class="section-head"><div><p class="eyebrow">Objetivos visuales</p><h2>Seleccioná qué querés comunicar</h2></div></div>
      <div class="grid two">${objectives.map(obj => `
        <article class="card">
          <p class="eyebrow">Objetivo</p>
          <h2>${esc(obj.title)}</h2>
          <p>${esc(obj.businessQuestion)}</p>
          <div class="pill-row">${(obj.recommendedCharts || []).map(c => `<span class="pill">${esc(c)}</span>`).join("")}</div>
          <a class="btn primary" href="#/storyboard-objective/${obj.id}">Construir storyboard</a>
        </article>`).join("")}</div>
      <div class="card section">
        <h2>Plantillas de dashboard</h2>
        <div class="grid three">${templates.map(tpl => `
          <article class="stat"><strong>${esc(tpl.name)}</strong><span>${esc(tpl.purpose)}</span><p><small>Ideal para: ${esc(tpl.bestFor)}</small></p></article>`).join("")}</div>
      </div>
      <div class="card section">
        <h2>Historial local</h2>
        ${history.length ? `<div class="list">${history.map(item => `<div class="list-row"><span><strong>${esc(item.title)}</strong><br><small>${esc(item.createdAt)}</small></span><span class="pill ok">${esc(item.template)}</span></div>`).join("")}</div>` : `<p class="muted">Todavía no guardaste storyboards. El historial no guarda datos sensibles: solo objetivo, plantilla y fecha.</p>`}
      </div>
    </section>`;
}

function buildStoryboardReport(obj, template) {
  const dataset = getVisualDataset(obj.recommendedDatasetId);
  return [
    `Storyboard visual: ${obj.title}`,
    `Pregunta de negocio: ${obj.businessQuestion}`,
    `Dataset sugerido: ${dataset?.title || obj.recommendedDatasetId}`,
    `Plantilla: ${template.name}`,
    `Tarjetas KPI sugeridas: ${(obj.recommendedCards || []).join(", ")}`,
    `Gráficos sugeridos: ${(obj.recommendedCharts || []).join(", ")}`,
    `Orden narrativo:`,
    ...(obj.storyOrder || []).map((step, index) => `${index + 1}. ${step}`),
    `Hallazgo de ejemplo: ${obj.sampleInsight}`,
    `Evitar: ${obj.avoid}`,
    `Límite educativo: esta práctica no conecta con Tableau real; sirve para aprender diseño, lectura y comunicación visual.`
  ].join("\n");
}

function renderStoryboardObjective(objectiveId) {
  const obj = getStoryboardObjective(objectiveId);
  if (!obj) return renderNotFound();
  const dataset = getVisualDataset(obj.recommendedDatasetId);
  const templates = dashboardStoryboard.dashboardTemplates || [];
  const selectedTemplate = templates[0] || { id: "template", name: "Dashboard simple", layout: [] };
  const report = buildStoryboardReport(obj, selectedTemplate);
  app.innerHTML = `
    <section class="article wide-article">
      <div class="card">
        <p class="eyebrow">Constructor de storyboard</p>
        <h1>${esc(obj.title)}</h1>
        <p class="lead">${esc(obj.businessQuestion)}</p>
        <div class="actions"><a class="btn ghost" href="#/dashboard-storyboard">Volver</a><button class="btn primary" id="copyStoryboard">Copiar reporte</button><button class="btn ghost" id="downloadStoryboard">Descargar .txt</button><button class="btn ghost" id="saveStoryboard">Guardar en historial</button></div>
      </div>
      <div class="grid two section">
        <article class="card">
          <p class="eyebrow">Dataset sugerido</p>
          <h2>${esc(dataset?.title || obj.recommendedDatasetId)}</h2>
          <p>${esc(dataset?.description || "Dataset de práctica")}</p>
          ${renderMiniTable(dataset)}
        </article>
        <article class="card ok-card">
          <p class="eyebrow">Tarjetas y gráficos</p>
          <h2>Elementos recomendados</h2>
          <h3>Tarjetas KPI</h3>${list(obj.recommendedCards || [])}
          <h3>Gráficos</h3>${list(obj.recommendedCharts || [])}
        </article>
      </div>
      <div class="grid two section">
        <article class="card">
          <h2>Orden narrativo</h2>
          <ol>${(obj.storyOrder || []).map(step => `<li>${esc(step)}</li>`).join("")}</ol>
        </article>
        <article class="card">
          <h2>Plantillas disponibles</h2>
          ${(templates || []).map(tpl => `<details class="details"><summary>${esc(tpl.name)}</summary>${list(tpl.layout || [])}<p>${esc(tpl.bestFor || "")}</p></details>`).join("")}
        </article>
      </div>
      <div class="card section">
        <h2>Reporte de storyboard</h2>
        <pre><code>${esc(report)}</code></pre>
      </div>
    </section>`;
  document.querySelector("#copyStoryboard")?.addEventListener("click", async () => { await navigator.clipboard.writeText(report); alert("Reporte copiado."); });
  document.querySelector("#downloadStoryboard")?.addEventListener("click", () => downloadText(`storyboard_tableau_${obj.id}.txt`, report));
  document.querySelector("#saveStoryboard")?.addEventListener("click", () => {
    const history = readStoryboardHistory();
    history.unshift({ id: obj.id, title: obj.title, template: selectedTemplate.name, createdAt: new Date().toLocaleString() });
    writeStoryboardHistory(history);
    alert("Storyboard guardado en historial local.");
  });
}

function renderStoryboardReport() {
  const history = readStoryboardHistory();
  app.innerHTML = `
    <section class="article wide-article">
      <div class="card">
        <p class="eyebrow">Historial y reporte</p>
        <h1>Storyboards practicados</h1>
        <p class="lead">Este historial es local y no guarda datos sensibles ni datasets completos.</p>
        <div class="actions"><a class="btn primary" href="#/dashboard-storyboard">Crear nuevo storyboard</a><button class="btn ghost" id="clearStoryboardHistory">Limpiar historial</button></div>
      </div>
      <div class="card section">
        ${history.length ? `<div class="list">${history.map(item => `<div class="list-row"><span><strong>${esc(item.title)}</strong><br><small>${esc(item.createdAt)}</small></span><span class="pill ok">${esc(item.template)}</span></div>`).join("")}</div>` : `<p>No hay storyboards guardados todavía.</p>`}
      </div>
    </section>`;
  document.querySelector("#clearStoryboardHistory")?.addEventListener("click", () => { writeStoryboardHistory([]); renderStoryboardReport(); });
}


function storytellingStorageKey() {
  const id = manifest?.courseId || "curso-web";
  return `curso-web:${id}:storytelling-history:v1`;
}

function readStorytellingHistory() {
  try { return JSON.parse(localStorage.getItem(storytellingStorageKey())) || []; } catch { return []; }
}

function writeStorytellingHistory(items) {
  localStorage.setItem(storytellingStorageKey(), JSON.stringify(items.slice(0, 12)));
}

function getStoryScenario(id) {
  return (storytellingLab.scenarios || []).find(item => item.id === id);
}

function buildStorytellingReport(scenario) {
  return [
    `Presentación visual: ${scenario.title}`,
    `Pregunta de negocio: ${scenario.businessQuestion}`,
    `Audiencia: ${scenario.audience}`,
    `Apertura sugerida: ${scenario.recommendedOpening}`,
    `Evidencia a mostrar:`,
    ...(scenario.evidence || []).map((item, index) => `${index + 1}. ${item}`),
    `Orden visual recomendado:`,
    ...(scenario.visualOrder || []).map((item, index) => `${index + 1}. ${item}`),
    `Mala práctica a evitar: ${scenario.badPractice}`,
    `Cierre recomendado: ${scenario.closingRecommendation}`,
    `Límite educativo: este simulador no reemplaza Tableau real; ayuda a preparar una explicación clara antes de construir el dashboard.`
  ].join("\n");
}

function renderStorytelling() {
  const scenarios = storytellingLab.scenarios || [];
  const history = readStorytellingHistory();
  app.innerHTML = `
    <section class="article wide-article">
      <div class="card">
        <p class="eyebrow">v0.4 · Storytelling visual</p>
        <h1>Explicar dashboards con claridad</h1>
        <p class="lead">Practicá cómo convertir gráficos en una explicación útil: pregunta, evidencia, hallazgo, límites y próximo paso.</p>
        <div class="actions">
          <a class="btn ghost" href="#/dashboard-storyboard">Constructor storyboard</a>
          <a class="btn ghost" href="#/misleading-visuals">Visualizaciones engañosas</a>
          <a class="btn ghost" href="#/visual-clarity-checklist">Checklist de claridad</a>
        </div>
      </div>
      <div class="section-head"><div><p class="eyebrow">Simulador</p><h2>Elegí una presentación visual</h2></div></div>
      <div class="grid two">${scenarios.map(s => `
        <article class="card">
          <p class="eyebrow">Presentación</p>
          <h2>${esc(s.title)}</h2>
          <p>${esc(s.businessQuestion)}</p>
          <p><strong>Audiencia:</strong> ${esc(s.audience)}</p>
          <a class="btn primary" href="#/visual-presentation/${s.id}">Practicar explicación</a>
        </article>`).join("")}</div>
      <div class="card section">
        <h2>Historial local</h2>
        ${history.length ? `<div class="list">${history.map(item => `<div class="list-row"><span><strong>${esc(item.title)}</strong><br><small>${esc(item.createdAt)}</small></span><span class="pill ok">practicado</span></div>`).join("")}</div>` : `<p class="muted">Todavía no guardaste presentaciones. El historial solo guarda título y fecha, no datos sensibles.</p>`}
      </div>
    </section>`;
}

function renderVisualPresentation(scenarioId) {
  const scenario = getStoryScenario(scenarioId);
  if (!scenario) return renderNotFound();
  const report = buildStorytellingReport(scenario);
  app.innerHTML = `
    <section class="article wide-article">
      <div class="card">
        <p class="eyebrow">Simulador de presentación</p>
        <h1>${esc(scenario.title)}</h1>
        <p class="lead">${esc(scenario.businessQuestion)}</p>
        <div class="actions"><a class="btn ghost" href="#/storytelling">Volver</a><button class="btn primary" id="copyStoryReport">Copiar reporte</button><button class="btn ghost" id="downloadStoryReport">Descargar .txt</button><button class="btn ghost" id="saveStoryReport">Guardar práctica</button></div>
      </div>
      <div class="grid two section">
        <article class="card ok-card">
          <p class="eyebrow">Apertura</p>
          <h2>Cómo empezar</h2>
          <p>${esc(scenario.recommendedOpening)}</p>
        </article>
        <article class="card danger-card">
          <p class="eyebrow">Evitar</p>
          <h2>Mala práctica</h2>
          <p>${esc(scenario.badPractice)}</p>
        </article>
      </div>
      <div class="grid two section">
        <article class="card"><h2>Evidencia a mostrar</h2>${list(scenario.evidence || [])}</article>
        <article class="card"><h2>Orden visual recomendado</h2><ol>${(scenario.visualOrder || []).map(step => `<li>${esc(step)}</li>`).join("")}</ol></article>
      </div>
      <div class="card section">
        <h2>Cierre sugerido</h2>
        <p>${esc(scenario.closingRecommendation)}</p>
      </div>
      <div class="card section">
        <h2>Reporte de presentación</h2>
        <pre><code>${esc(report)}</code></pre>
      </div>
    </section>`;
  document.querySelector("#copyStoryReport")?.addEventListener("click", async () => { await navigator.clipboard.writeText(report); alert("Reporte copiado."); });
  document.querySelector("#downloadStoryReport")?.addEventListener("click", () => downloadText(`storytelling_tableau_${scenario.id}.txt`, report));
  document.querySelector("#saveStoryReport")?.addEventListener("click", () => {
    const history = readStorytellingHistory();
    history.unshift({ id: scenario.id, title: scenario.title, createdAt: new Date().toLocaleString() });
    writeStorytellingHistory(history);
    alert("Práctica guardada en historial local.");
  });
}

function renderMisleadingVisuals() {
  const examples = storytellingLab.misleadingExamples || [];
  app.innerHTML = `
    <section class="article wide-article">
      <div class="card">
        <p class="eyebrow">Buenas y malas prácticas</p>
        <h1>Visualizaciones engañosas o confusas</h1>
        <p class="lead">No todo gráfico bonito comunica bien. Revisá errores comunes antes de publicar un dashboard.</p>
        <div class="actions"><a class="btn ghost" href="#/storytelling">Volver a storytelling</a><a class="btn primary" href="#/visual-clarity-checklist">Abrir checklist</a></div>
      </div>
      <div class="grid two section">${examples.map(ex => `
        <article class="card danger-card">
          <p class="eyebrow">Error visual</p>
          <h2>${esc(ex.title)}</h2>
          <p><strong>Problema:</strong> ${esc(ex.problem)}</p>
          <p><strong>Mejor alternativa:</strong> ${esc(ex.better)}</p>
        </article>`).join("")}</div>
    </section>`;
}

function renderVisualClarityChecklist() {
  const items = storytellingLab.clarityChecklist || [];
  const report = [`Checklist de claridad visual`, ...items.map((item, index) => `${index + 1}. ${item}`)].join("\n");
  app.innerHTML = `
    <section class="article">
      <div class="card">
        <p class="eyebrow">Checklist v0.4</p>
        <h1>Claridad visual antes de compartir</h1>
        <p class="lead">Usá esta lista antes de presentar un dashboard o storyboard.</p>
        <div class="actions"><a class="btn ghost" href="#/storytelling">Volver</a><button class="btn primary" id="copyClarity">Copiar checklist</button><button class="btn ghost" onclick="window.print()">Imprimir / PDF</button></div>
      </div>
      <div class="card section"><h2>Checklist</h2>${list(items)}</div>
    </section>`;
  document.querySelector("#copyClarity")?.addEventListener("click", async () => { await navigator.clipboard.writeText(report); alert("Checklist copiado."); });
}
