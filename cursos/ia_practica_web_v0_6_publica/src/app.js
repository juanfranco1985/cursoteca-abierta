...
let checklists = [];
let incidents = [];
let promptLibrary = { categories: [], labExercises: [] };
...
function applyManifestToShell() {
  ...
  const promptsNav = document.querySelector("[data-nav-prompts]");
  if (promptsNav) promptsNav.textContent = manifest.labels?.prompts || "Prompts";
}
...
async function bootstrap() {
  try {
    ...
    const [courseData, checklistData, incidentData, promptData] = await Promise.all([
      loadJson(manifest.dataPaths.course),
      loadJson(manifest.dataPaths.checklists),
      loadJson(manifest.dataPaths.incidents),
      manifest.dataPaths.prompts ? loadJson(manifest.dataPaths.prompts) : Promise.resolve({ categories: [], labExercises: [] }),
    ]);
    course = courseData;
    checklists = checklistData.checklists || [];
    incidents = incidentData.incidents || [];
    promptLibrary = promptData || { categories: [], labExercises: [] };
...
function render() {
  ...
  const routes = {
    ...
    "prompt": () => renderPromptDetail(p1, p2),
    "lab": renderPracticeLab,
    "builder": renderPromptBuilder,
  ...
  app.innerHTML = `
    ...
    <section class="section grid two">
      <div class="card accent-card">
        <p class="eyebrow">Nuevo en v0.2</p>
        <h2>Biblioteca de prompts copiables</h2>
        <p>Usá prompts editables para clientes, resúmenes, ventas, organización, estudio y verificación. Cada prompt incluye advertencias de privacidad.</p>
        <a class="btn primary" href="#/prompts">Abrir biblioteca</a>
      </div>
      <div class="card">
        <p class="eyebrow">Laboratorio de práctica</p>
        <h2>Ejercicios guiados</h2>
        <p>Practicá con tareas concretas y criterios de éxito antes de usar IA en un caso real.</p>
        <a class="btn ghost" href="#/lab">Abrir laboratorio</a>
      </div>
      <div class="card accent-card">
        <p class="eyebrow">Nuevo en v0.3</p>
        <h2>Constructor de prompts</h2>
        <p>Completá objetivo, contexto, tono, formato y límites para generar un prompt propio, ordenado y seguro.</p>
        <a class="btn primary" href="#/builder">Abrir constructor</a>
      </div>
    </section>
    ...
    <section class="section card compact-helper">
      <div>
        <p class="eyebrow">v0.3 del curso IA</p>
        <h2>Prompts, práctica y constructor propio</h2>
        <p>Se agregaron ejemplos prellenables, exportación TXT, rúbrica de calidad y mejoras del laboratorio de práctica.</p>
      </div>
...
function getPromptCategory(categoryId) {
  return (promptLibrary.categories || []).find(category => category.id === categoryId);
}

function getPrompt(categoryId, promptId) {
  return getPromptCategory(categoryId)?.prompts?.find(prompt => prompt.id === promptId);
}
...
function renderPrompts() {
  ...
  app.innerHTML = `
    <section class="section-head">
      <div>
        <p class="eyebrow">Biblioteca v0.2</p>
        <h1>${esc(manifest.promptLibrary?.title || "Prompts copiables")}</h1>
        <p>${esc(manifest.promptLibrary?.description || "Prompts prácticos para usar IA con más criterio.")}</p>
      </div>
      <a class="btn ghost" href="#/lab">Ir al laboratorio</a>
    </section>
    <section class="card compact-helper">
      <div>
        <p class="eyebrow">Uso responsable</p>
        <h2>Antes de copiar</h2>
        <p>${esc(manifest.promptLibrary?.copyNotice || "Reemplazá los campos entre corchetes y no pegues información privada.")}</p>
      </div>
    </section>
    <input class="search-box" id="promptSearch" placeholder="Buscar: cliente, resumen, ventas, agenda, estudiar, verificar…" aria-label="Buscar prompts" />
    <section class="grid two" id="promptGrid"></section>`;
  ...
  const paint = () => {
    ...
    for (const category of categories) {
      const prompts = (category.prompts || []).filter(prompt => `${category.title} ${category.description} ${prompt.title} ${prompt.useCase} ${prompt.template}`.toLowerCase().includes(q));
      if (!prompts.length) continue;
      cards.push(`
        <article class="card prompt-category-card">
          <span class="pill">${esc(category.title)}</span>
          <h2>${esc(category.title)}</h2>
          <p>${esc(category.description)}</p>
          <div class="list section">
            ${prompts.map(prompt => `<a class="list-row" href="#/prompt/${category.id}/${prompt.id}"><span><strong>${esc(prompt.title)}</strong><br><small>${esc(prompt.useCase)}</small></span><span class="pill ok">Copiar</span></a>`).join("")}
          </div>
        </article>`);
    }
    grid.innerHTML = cards.join("") || `<div class="card"><p>No encontré prompts con esa búsqueda.</p></div>`;
  };
  search.addEventListener("input", paint);
  paint();
}

function renderPromptDetail(categoryId, promptId) {
  const category = getPromptCategory(categoryId);
  const prompt = getPrompt(categoryId, promptId);
  if (!category || !prompt) return renderNotFound();
  app.innerHTML = `
    <article class="article">
      <div class="card">
        <p class="eyebrow">${esc(category.title)}</p>
        <h1>${esc(prompt.title)}</h1>
        <p class="lead">${esc(prompt.useCase)}</p>
        <div class="actions"><button class="btn primary" id="copyPrompt">Copiar prompt</button><a class="btn ghost" href="#/prompts">Volver a biblioteca</a><a class="btn ghost" href="#/lab">Practicar</a><a class="btn ghost" href="#/builder">Crear prompt propio</a></div>
        <p id="copyPromptMsg" class="muted"></p>
      </div>
      <div class="card prompt-template">
        <h2>Prompt base</h2>
        <textarea class="textarea prompt-copy-box" id="promptText">${esc(prompt.template)}</textarea>
      </div>
      <div class="card"><h2>Cómo usarlo mejor</h2>${list(prompt.tips)}</div>
      <div class="card alert"><h2>Privacidad y límites</h2><p>${esc(prompt.privacyWarning || manifest.responsibleNotice || "No compartas datos sensibles.")}</p></div>
      <div class="card"><h2>Mini práctica sugerida</h2><p>Copiá el prompt, reemplazá los campos entre corchetes y probalo con un ejemplo ficticio antes de usarlo con información real.</p></div>
    </article>`;
  document.querySelector("#copyPrompt").addEventListener("click", async () => {
    const value = document.querySelector("#promptText").value;
    try {
      await navigator.clipboard?.writeText(value);
      document.querySelector("#copyPromptMsg").textContent = "Prompt copiado. Revisá y reemplazá los campos entre corchetes antes de usarlo.";
    } catch {
      document.querySelector("#copyPromptMsg").textContent = "No se pudo copiar automáticamente. Seleccioná el texto y copialo manualmente.";
    }
  });
}

function renderPracticeLab() {
  const exercises = promptLibrary.labExercises || [];
  app.innerHTML = `
    <section class="article">
      <div class="card">
        <p class="eyebrow">Laboratorio v0.4</p>
        <h1>Laboratorio de práctica con IA</h1>
        <p class="lead">Ejercicios cortos con práctica guiada, criterios de éxito y vínculo directo con el constructor de prompts.</p>
        <div class="actions"><a class="btn primary" href="#/prompts">Abrir biblioteca de prompts</a><a class="btn ghost" href="#/builder">Crear prompt propio</a><a class="btn ghost" href="#/modules">Volver al curso</a></div>
      </div>
      ${exercises.map((exercise, index) => `
        <article class="card lab-card enhanced-lab-card">
          <p class="eyebrow">Ejercicio ${index + 1}</p>
          <h2>${esc(exercise.title)}</h2>
          <p>${esc(exercise.objective)}</p>
          <div class="lab-columns">
            <div><h3>Pasos</h3>${list(exercise.steps)}</div>
            <div><h3>Criterios de éxito</h3>${list(exercise.successCriteria)}</div>
          </div>
          <div class="actions"><a class="btn ghost" href="#/builder">Crear un prompt para este ejercicio</a><a class="btn ghost" href="#/prompts">Buscar prompt base</a></div>
        </article>`).join("")}
      <div class="card alert"><h2>Regla del laboratorio</h2><p>Usá ejemplos ficticios o anonimizados. La práctica vale más si después verificás la respuesta, la corregís y la adaptás a tu contexto.</p></div>
    </section>`;
}
...
function promptDraftKey() {
  const id = manifest?.courseId || "curso-web";
  return `curso-web:${id}:prompt-builder:v1`;
}

function defaultPromptDraft() {
  return {
    objective: "",
    context: "",
    audience: "",
    tone: "claro, profesional y simple",
    outputFormat: "lista de pasos claros",
    constraints: "no inventes datos; si falta información, indicá qué debería confirmar",
    privacy: "no incluir datos personales, claves, números de cuenta ni información sensible",
    verification: "marcá supuestos, riesgos y puntos que debo verificar antes de usar la respuesta"
  };
}

function readPromptDraft() {
  try {
    return { ...defaultPromptDraft(), ...(JSON.parse(localStorage.getItem(promptDraftKey())) || {}) };
  } catch {
    return defaultPromptDraft();
  }
}

function savePromptDraft(draft) {
  localStorage.setItem(promptDraftKey(), JSON.stringify({ ...draft, updatedAt: new Date().toISOString() }));
}

function buildPromptFromDraft(draft) {
  const clean = (value, fallback) => String(value || fallback || "").trim();
  const sections = [
    `Actuá como un asistente experto y práctico.`,
    `Objetivo: ${clean(draft.objective, "ayudarme a resolver una tarea concreta")}.`,
    `Contexto: ${clean(draft.context, "todavía no agregué contexto; pedime los datos que falten antes de asumir")}.`,
    `Público o destinatario: ${clean(draft.audience, "usuario general")}.`,
    `Tono: ${clean(draft.tone, "claro, profesional y simple")}.`,
    `Formato de salida: ${clean(draft.outputFormat, "respuesta ordenada en pasos")}.`,
    `Restricciones: ${clean(draft.constraints, "no inventes datos; declarà supuestos y límites")}.`,
    `Privacidad: ${clean(draft.privacy, "no uses ni solicites datos sensibles innecesarios")}.`,
    `Verificación: ${clean(draft.verification, "incluí puntos que debería revisar antes de actuar")}.`,
    `Antes de responder, si detectás ambigüedades importantes, indicá qué información falta.`
  ];
  return sections.join("\n");
}


function promptBuilderExamples() {
  return Array.isArray(manifest.promptBuilder?.examples) ? manifest.promptBuilder.examples : [];
}

function promptQualityRubric() {
  return Array.isArray(manifest.promptBuilder?.qualityRubric) ? manifest.promptBuilder.qualityRubric : [];
}

function scorePromptDraft(draft, promptText = "") {
  const text = `${promptText}\n${Object.values(draft || {}).join("\n")}`.toLowerCase();
  const checks = [
    { id: "rubric_objective", ok: Boolean(String(draft.objective || "").trim().length >= 12) },
    { id: "rubric_context", ok: Boolean(String(draft.context || "").trim().length >= 25) },
    { id: "rubric_format", ok: Boolean(String(draft.outputFormat || "").trim().length >= 8) },
    { id: "rubric_constraints", ok: /no invent|restric|l[ií]mit|promet|asum/.test(text) },
    { id: "rubric_privacy", ok: /privacidad|datos personales|sensible|clave|bancari|dni|tel[eé]fono/.test(text) },
    { id: "rubric_verification", ok: /verific|confirm|supuesto|revis|fuente|validar/.test(text) }
  ];
  const passed = checks.filter(item => item.ok).length;
  const percent = Math.round((passed / checks.length) * 100);
  return { percent, passed, total: checks.length, checks };
}

function renderPromptRubric(draft, promptText) {
  const rubric = promptQualityRubric();
  const score = scorePromptDraft(draft, promptText);
  const byId = Object.fromEntries(score.checks.map(item => [item.id, item.ok]));
  return `
    <div class="rubric-score">
      <strong>${score.percent}%</strong>
      <span>${score.passed}/${score.total} criterios cubiertos</span>
      ${progressBar(score.percent)}
    </div>
    <div class="rubric-list">
      ${rubric.map(item => `<div class="rubric-item ${byId[item.id] ? "ok" : "warn"}"><span>${byId[item.id] ? "✓" : "!"}</span><div><strong>${esc(item.title)}</strong><br><small>${esc(item.description)}</small></div></div>`).join("")}
    </div>`;
}
...
function renderPromptBuilder() {
  ...
  const examples = promptBuilderExamples();
  const promptText = buildPromptFromDraft(draft);
  app.innerHTML = `
    <section class="article wide-article">
      <div class="card">
        <p class="eyebrow">Constructor v0.4</p>
        <h1>Constructor de prompts con rúbrica</h1>
        <p class="lead">Completá los campos clave, cargá ejemplos prellenables, revisá la calidad del prompt y exportalo como archivo .txt.</p>
        <div class="actions"><a class="btn ghost" href="#/prompts">Ver biblioteca</a><a class="btn ghost" href="#/lab">Ir al laboratorio</a></div>
      </div>
      <div class="card builder-examples-card">
        <p class="eyebrow">Ejemplos prellenables</p>
        <h2>Cargar una base rápida</h2>
        <p>Elegí un caso y después ajustalo con tus datos reales, sin pegar información sensible.</p>
        <div class="actions example-actions">
          ${examples.map(example => `<button type="button" class="btn ghost" data-builder-example="${esc(example.id)}">${esc(example.title)}</button>`).join("")}
        </div>
      </div>
      <div class="grid two prompt-builder-grid">
        <form class="card prompt-builder-form" id="promptBuilderForm">
          <label>Objetivo principal
            <textarea name="objective" rows="3" placeholder="Ej.: responder a un cliente molesto sin sonar agresivo">${esc(draft.objective)}</textarea>
          </label>
          <label>Contexto
            <textarea name="context" rows="4" placeholder="Ej.: el cliente compró ayer, el envío se demoró y reclama por WhatsApp">${esc(draft.context)}</textarea>
          </label>
          <label>Público o destinatario
            <input name="audience" value="${esc(draft.audience)}" placeholder="Ej.: cliente minorista, equipo interno, profesor, proveedor" />
          </label>
          <label>Tono
            <input name="tone" value="${esc(draft.tone)}" placeholder="Ej.: amable, breve, profesional, didáctico" />
          </label>
          <label>Formato de salida
            <input name="outputFormat" value="${esc(draft.outputFormat)}" placeholder="Ej.: tabla, lista de pasos, mensaje listo para copiar" />
          </label>
          <label>Restricciones
            <textarea name="constraints" rows="3" placeholder="Ej.: no inventar precios, no prometer resultados, no usar lenguaje técnico">${esc(draft.constraints)}</textarea>
          </label>
          <label>Datos que no se deben incluir
            <textarea name="privacy" rows="3" placeholder="Ej.: no incluir DNI, claves, datos bancarios, direcciones ni teléfonos reales">${esc(draft.privacy)}</textarea>
          </label>
          <label>Verificación requerida
            <textarea name="verification" rows="3" placeholder="Ej.: marcar supuestos, revisar fechas, confirmar datos importantes">${esc(draft.verification)}</textarea>
          </label>
          <div class="actions">
            <button type="button" class="btn primary" id="updatePromptPreview">Actualizar prompt</button>
            <button type="button" class="btn ghost" id="resetPromptBuilder">Limpiar</button>
          </div>
          <p class="muted">El borrador se guarda solo en este navegador con LocalStorage.</p>
        </form>
        <aside class="builder-side-panel">
          <article class="card prompt-preview-card">
            <p class="eyebrow">Prompt generado</p>
            <h2>Vista previa editable</h2>
            <textarea id="promptBuilderOutput" rows="18" class="prompt-copy-box">${esc(promptText)}</textarea>
            <div class="actions">
              <button class="btn primary" id="copyBuiltPrompt">Copiar prompt</button>
              <button class="btn ghost" id="saveBuiltPrompt">Guardar borrador</button>
              <button class="btn ghost" id="downloadBuiltPrompt">Exportar .txt</button>
            </div>
            <p id="builderCopyStatus" class="muted"></p>
          </article>
          <article class="card rubric-card" id="promptRubricPanel">
            <p class="eyebrow">Rúbrica de calidad</p>
            <h2>Revisión rápida</h2>
            ${renderPromptRubric(draft, promptText)}
          </article>
        </aside>
      </div>
      <div class="card alert">
        <h2>Regla de seguridad</h2>
        <p>Antes de pegar un prompt en una IA externa, revisá que no incluya datos personales, claves, información bancaria, documentos privados ni datos sensibles de clientes o terceros.</p>
      </div>
    </section>`;

  const form = document.querySelector("#promptBuilderForm");
  const output = document.querySelector("#promptBuilderOutput");
  const status = document.querySelector("#builderCopyStatus");
  const rubricPanel = document.querySelector("#promptRubricPanel");
  const collect = () => Object.fromEntries(new FormData(form).entries());
  const repaintRubric = () => {
    const current = { ...defaultPromptDraft(), ...collect() };
    if (rubricPanel) rubricPanel.innerHTML = `<p class="eyebrow">Rúbrica de calidad</p><h2>Revisión rápida</h2>${renderPromptRubric(current, output.value)}`;
  };
  const updatePreview = (message = "Vista previa actualizada.") => {
    const next = { ...defaultPromptDraft(), ...collect() };
    savePromptDraft(next);
    output.value = buildPromptFromDraft(next);
    repaintRubric();
    if (status) status.textContent = message;
  };
  const fillForm = (nextDraft) => {
    for (const [key, value] of Object.entries(nextDraft)) {
      const input = form.elements[key];
      if (input) input.value = value;
    }
    updatePreview("Ejemplo cargado. Ajustalo antes de usarlo.");
  };
  document.querySelectorAll("[data-builder-example]").forEach(button => {
    button.addEventListener("click", () => {
      const example = examples.find(item => item.id === button.dataset.builderExample);
      if (example?.draft) fillForm({ ...defaultPromptDraft(), ...example.draft });
    });
  });
  form?.addEventListener("input", () => updatePreview("Borrador actualizado."));
  output?.addEventListener("input", repaintRubric);
  document.querySelector("#updatePromptPreview")?.addEventListener("click", () => updatePreview());
  document.querySelector("#saveBuiltPrompt")?.addEventListener("click", () => {
    savePromptDraft({ ...defaultPromptDraft(), ...collect(), customOutput: output.value });
    if (status) status.textContent = "Borrador guardado en este navegador.";
  });
  document.querySelector("#resetPromptBuilder")?.addEventListener("click", () => {
    localStorage.removeItem(promptDraftKey());
    renderPromptBuilder();
  });
  document.querySelector("#downloadBuiltPrompt")?.addEventListener("click", () => {
    const stamp = new Date().toISOString().slice(0, 10);
    downloadTextFile(`prompt-ia-practica-${stamp}.txt`, output.value);
    if (status) status.textContent = "Archivo .txt generado desde el navegador.";
  });
  document.querySelector("#copyBuiltPrompt")?.addEventListener("click", async () => {
    try {
      await navigator.clipboard?.writeText(output.value);
      if (status) status.textContent = "Prompt copiado al portapapeles.";
    } catch {
      if (status) status.textContent = "No se pudo copiar automáticamente. Seleccioná el texto y copiá manualmente.";
    }
  });
}
