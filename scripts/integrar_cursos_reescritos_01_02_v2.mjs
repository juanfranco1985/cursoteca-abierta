...
const extractedRoot = path.join(workspaceRoot, "__cursos_reescritos_01_02_v2");
const repoSourceRoot = path.join(root, "contenido_reescrito", "cursos_01_02_v2");
const coursesRoot = path.join(root, "cursos");
const date = "2026-06-07";
const version = "1.1-curso-completo-v2";
const publicVersion = "v2 curso completo";
const status = "Curso completo v2";
const sourcePackage = "cursos_01_02_reescritos_v2";
const depthStatus = "curso-completo-v2";
const publicationStatus = "publica-curso-completo-v2";
const contentDepthLabel = "curso completo reescrito v2";
const cacheSuffix = "cache-v2-curso-completo";
...
function sourceDiscoveryRoot() {
  return fs.existsSync(extractedRoot) ? extractedRoot : repoSourceRoot;
}
...
function tokenSet(value) {
  const ignored = new Set(["v0", "v1", "6", "publica", "web", "para", "por", "con", "de", "del", "desde", "los", "las", "una", "uno", "curso"]);
  return new Set(slugify(value).split("_").filter(token => token.length > 1 && !ignored.has(token)));
}

function matchCourseFolder(sourceSlug, courseFolders, usedFolders) {
  const sourceTokens = tokenSet(sourceSlug);
  const candidates = courseFolders
    .filter(folder => !usedFolders.has(folder))
    .map(folder => {
      const folderCore = folder.replace(/_v0_6_publica$/, "");
      const folderTokens = tokenSet(folderCore);
      const shared = [...sourceTokens].filter(token => folderTokens.has(token));
      const exactPrefix = folderCore === sourceSlug || folderCore.includes(sourceSlug) || sourceSlug.includes(folderCore);
      const score = shared.length * 10 + (exactPrefix ? 100 : 0) - Math.abs(folderTokens.size - sourceTokens.size);
      return { folder, score, shared };
    })
    .sort((a, b) => b.score - a.score);
  const best = candidates[0];
  if (!best || best.score < 10) {
    throw new Error(`No se pudo mapear el paquete ${sourceSlug} a una carpeta de curso publicada.`);
  }
  return best.folder;
}

function courseTitleFromMarkdown(markdown, fallback) {
  const heading = normalizeText(markdown).match(/^#\s+(.+)$/m)?.[1] || fallback;
  return heading.replace(/^Curso\s+\d+\s+[—-]\s*/i, "").trim();
}

function descriptionFromMarkdown(markdown, title) {
  const presentation = sectionContent(splitTopSections(markdown), /^Presentaci[oó]n/i);
  const clean = firstSentences(presentation || markdown, 2);
  return clean || `Curso completo para estudiar ${title} con teoria, practica, evaluacion y producto final.`;
}

function finalArtifactFromMarkdown(markdown, title) {
  const product = parseProduct(markdown);
  const clean = stripMarkdown(product).replace(/\s+/g, " ").trim();
  const match = clean.match(/(?:construir|crear|entregar|elaborar|desarrollar)\s+(?:su\s+|una\s+|un\s+|el\s+|la\s+)?([^.!?]{8,90})/i);
  if (match) return words(match[1], 14).replace(/\.$/, "");
  return `Producto final aplicado de ${title}`;
}

function buildCourseInputs() {
  const discoveryRoot = sourceDiscoveryRoot();
  if (!fs.existsSync(discoveryRoot)) {
    throw new Error(`No existe la carpeta fuente ${discoveryRoot}`);
  }
  const courseFolders = fs.readdirSync(coursesRoot, { withFileTypes: true })
    .filter(entry => entry.isDirectory())
    .map(entry => entry.name);
  const packageFolders = fs.readdirSync(discoveryRoot, { withFileTypes: true })
    .filter(entry => entry.isDirectory() && /^cursoteca_curso_0[12]_reescrito_v2$/.test(entry.name))
    .map(entry => entry.name)
    .sort((a, b) => Number(a.match(/curso_(\d+)/)?.[1] || 0) - Number(b.match(/curso_(\d+)/)?.[1] || 0));
  const usedFolders = new Set();
  const inputs = packageFolders.map(sourceFolder => {
    const sourceDir = path.join(discoveryRoot, sourceFolder);
    const sourceName = fs.readdirSync(sourceDir)
      .find(name => /^curso_\d+_.+\.md$/i.test(name) && name.toLowerCase() !== "readme.md");
    if (!sourceName) throw new Error(`No se encontro Markdown de curso en ${sourceFolder}`);
    const sourceSlug = sourceName.replace(/^curso_\d+_/i, "").replace(/\.md$/i, "");
    const folder = matchCourseFolder(sourceSlug, courseFolders, usedFolders);
    usedFolders.add(folder);
    const markdown = fs.readFileSync(path.join(sourceDir, sourceName), "utf8");
    const title = courseTitleFromMarkdown(markdown, sourceSlug);
    return {
      folder,
      source: `${sourceFolder}/${sourceName}`,
      sourceFolder,
      finalArtifact: finalArtifactFromMarkdown(markdown, title),
      description: descriptionFromMarkdown(markdown, title),
      tags: uniq([depthStatus, "curso-completo-v1", ...slugify(title).split("_").filter(token => token.length > 2).slice(0, 5)])
    };
  });
  if (inputs.length !== 2) {
    throw new Error(`Se esperaban 2 cursos fuente v2 del 01 al 02 y se encontraron ${inputs.length}.`);
  }
  return inputs;
}
...
function sourceObjects(sources) {
  return sources.map(name => {
    const url = String(name).match(/https?:\/\/\S+/)?.[0]?.replace(/[).,;]+$/, "");
    const cleanName = String(name).replace(/https?:\/\/\S+/g, "").replace(/\s+[—-]\s*$/, "").trim();
    const title = cleanName || url || "Fuente recomendada";
    return {
      title,
      organization: title.split(/\s+[—-]\s+/)[0] || title,
      url: url || sourceUrls.get(title) || "#",
      note: "Fuente recomendada por el paquete de reescritura v2."
    };
  });
}
...
function updateManifestFiles(input, content, checklistCount, caseCount) {
  ...
  Object.assign(manifest, {
    ...
    webApp: {
      ...(manifest.webApp || {}),
      cacheName: `${input.folder}-${cacheSuffix}`,
      publicationStatus
    },
    professionalization: {
      tier: depthStatus,
  ...
  Object.assign(publication, {
    ...
    status,
    contentDepth: contentDepthLabel,
    updatedAt: date
  });
  ...
  if (fs.existsSync(serviceWorkerFile)) {
    const sw = fs.readFileSync(serviceWorkerFile, "utf8").replace(/const CACHE_NAME = "[^"]+";/, `const CACHE_NAME = "${input.folder}-${cacheSuffix}";`);
  ...
  if (fs.existsSync(readmeFile)) {
    ...
    const note = `\n\n## Reescritura curso completo v2\n\nActualizado el ${date} con version extendida, teoria ampliada, objetivos, modulos, caso integrador, actividades, evaluacion, glosario, producto final y fuentes recomendadas.\n`;
    fs.writeFileSync(readmeFile, readme.includes("## Reescritura curso completo v2") ? readme : `${readme.trim()}${note}`, "utf8");
  }
}
...
function updatePortalAndInventory(results) {
  ...
  for (const course of portalCourses) {
    ...
    Object.assign(course, {
      ...
      recommendedBase: "Curso reescrito con estructura completa v2: teoria extendida, practica, evaluacion y producto final.",
...
function updateRoadmap(results) {
  ...
  const marker = "## Cursos completos v2 incorporados 01-02";
  const block = `${marker}\n\nEl ${date} se integraron las versiones extendidas v2 de los cursos 01 y 02 para alinearlos con la profundidad del resto de la coleccion:\n\n${lines}\n\nEstos cursos reemplazan las versiones v1 mas cortas en sus JSON de curso y quedan marcados como \`${status}\` en portal e inventario.\n`;
  const markerIndex = text.indexOf(marker);
  if (markerIndex >= 0) text = text.slice(0, markerIndex).trimEnd();
  text = `${text.trimEnd()}\n\n${block}`;
