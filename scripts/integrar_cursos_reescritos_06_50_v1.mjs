...
const extractedRoot = path.join(workspaceRoot, "__cursos_reescritos_06_50_v1");
const repoSourceRoot = path.join(root, "contenido_reescrito", "cursos_06_50_v1");
...
const sourcePackage = "cursos_06_50_reescritos_v1";
...
function buildCourseInputs() {
  ...
  const packageFolders = fs.readdirSync(discoveryRoot, { withFileTypes: true })
    .filter(entry => entry.isDirectory() && /^cursoteca_curso_\d+_reescrito_v1$/.test(entry.name))
    .map(entry => entry.name)
  ...
  const inputs = packageFolders.map(sourceFolder => {
    ...
    return {
      ...
      tags: uniq(["curso-completo-v1", ...slugify(title).split("_").filter(token => token.length > 2).slice(0, 5)])
    };
  });
  if (inputs.length !== 45) {
    throw new Error(`Se esperaban 45 cursos fuente del 06 al 50 y se encontraron ${inputs.length}.`);
  }
  return inputs;
}
...
function updateRoadmap(results) {
  ...
  const marker = "## Cursos completos v1 incorporados 06-50";
  const block = `${marker}\n\nEl ${date} se integraron los 45 cursos reescritos del 06 al 50 que completan la norma de curso completo:\n\n${lines}\n\nEstos cursos reemplazan la estructura templada anterior en sus JSON de curso y quedan marcados como \`${status}\` en portal e inventario.\n`;
